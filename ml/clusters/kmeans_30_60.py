...
print("USER CLUSTERING ANALYSIS - 3 CLUSTERS (30min/60min)")
...
for file_idx, file in enumerate(files):
    ...
    print("\n" + "="*80)
    print(f"PROCESSING: {window_type.upper()} WINDOW (k={n_clusters})")
    ...
    # Add heart rate features (no window prefix)
    hr_features = [
    ...
    # Add HR features (use endswith to avoid matching 'min' inside 'minute')
    hr_mean_feat = None
    hr_std_feat = None
    for feat in hr_features:
    ...
    # 5. ELBOW METHOD (show why k clusters chosen)
    ...
    for k in k_range:
        ...
        cluster_labels_temp = kmeans_temp.fit_predict(X_users_scaled)
        inertias.append(kmeans_temp.inertia_)
        sil_score = silhouette_score(X_users_scaled, cluster_labels_temp)
    ...
    axes[0].axvline(x=n_clusters, color='r', linestyle='--', linewidth=2, label=f'k={n_clusters} (chosen)')
    ...
    axes[1].axvline(x=n_clusters, color='r', linestyle='--', linewidth=2, label=f'k={n_clusters} (chosen)')
    ...
    plt.suptitle(f'Optimal Number of Clusters - {window_type} window (k={n_clusters} chosen)', fontsize=14, fontweight='bold')
    plt.tight_layout()
    ...
    # Define cluster colors (supports up to 7 clusters)
    all_cluster_colors = ['#FF6B6B', '#4ECDC4', '#45B7D1', '#95E1D3', '#F38181', '#DDA0DD', '#87CEEB']
    cluster_colors = all_cluster_colors[:n_clusters]
    ...
    ax.set_title(f'User Clusters - PCA Visualization\n{window_type} window ({n_clusters} clusters, {len(user_profiles)} users)',
    ...
    for idx, feat in enumerate(user_feature_cols):
        ...
        for cluster_id in range(n_clusters):
            ...
            cluster_labels_box.append(f'C{cluster_id}')

        bp = ax.boxplot(cluster_data_list, labels=cluster_labels_box, patch_artist=True,
    ...
    cluster_sizes = [np.sum(user_clusters == i) for i in range(n_clusters)]
