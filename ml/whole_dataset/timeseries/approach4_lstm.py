...
def time_based_split(X, y, user_ids, train_ratio=0.70, val_ratio=0.15):
    ...
    user_ids_train = user_ids[train_idx]
    ...
    # Check for user overlap (important for time series)
    train_users = set(user_ids_train)
    val_users = set(user_ids_val)
    test_users = set(user_ids_test)
    ...
    print(f"  Note: This is normal for time-based splits where we split by time, not by user")
...
def train_and_evaluate(df, target_col, target_name, output_dir):
    ...
    X, y, user_ids, median = create_sequences_median(df, target_col, temporal_features, sequence_length=3)
    
    # 2. TIME-BASED split (70/15/15) - NO SHUFFLING
    X_train, X_val, X_test, y_train, y_val, y_test = time_based_split(
        X, y, user_ids, train_ratio=0.70, val_ratio=0.15
    )
